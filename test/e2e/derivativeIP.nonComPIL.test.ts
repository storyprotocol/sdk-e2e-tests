import { privateKeyA, privateKeyB, accountB, privateKeyC, accountC, nftContractAddress, accountA } from '../../config/config'
import { mintNFTWithRetry } from '../../utils/utils'
import { registerIpAsset, registerNonComSocialRemixingPIL, mintLicenseTokens, attachLicenseTerms, registerDerivative, registerDerivativeWithLicenseTokens } from '../../utils/sdkUtils'
...
            });
        ...
        describe.skip("Reciprocally register derivative IP asset with license token", async function () {
            step("Mint a NFT to Wallet A and get a tokenId (tokenIdA)", async function () {
                tokenIdA = await mintNFTWithRetry(privateKeyA);
                if (tokenIdA === "") {
                    throw new Error("Unable to mint NFT");
            ...
            step("Wallet A can register a derivative IP asset with licenseTokenIdA", async function () {
                const response = await expect(
                    registerDerivativeWithLicenseTokens("A", ipIdA, [licenseTokenIdB], waitForTransaction)
                ).to.not.be.rejected;
        ...
        describe.skip("Reciprocally register derivative IP asset without license token", async function () {
            step("Mint a NFT to Wallet A and get a tokenId (tokenIdA)", async function () {
                tokenIdA = await mintNFTWithRetry(privateKeyA);
                if (tokenIdA === "") {
                    throw new Error("Unable to mint NFT");
            ...
            step("Wallet B can register a derivative IP asset with ipId and LicenseTermsId", async function () {
                // const response = await expect(
                const response = await
...
            });
            ...
            step("Wallet A can register a derivative IP asset with ipId and LicenseTermsId", async function () {
                const response = await expect(
                    registerDerivative("A", ipIdA, [ipIdB], [licenseTermsId1], waitForTransaction)
                ).to.not.be.rejected;
