import { privateKeyA, nftContractAddress, accountA, accountB, licenseTokenAddress, privateKeyB } from '../../config/config';
import { attachLicenseTerms, ipAccountExecute, mintLicenseTokens, registerDerivativeIp, registerDerivativeWithLicenseTokens, registerIpAndAttachPilTerms, registerIpAsset } from '../../utils/sdkUtils';
...
import { transferLicenseTokenAbi } from '../../config/abi';
    ...
    describe(`Non-Commercial Social Remixing PIL: "transferable":false, "derivativesAllowed":false`, async function () {
        ...
        step(`"transferable":false, cannot transfer licenseTokenId to another account as LicenseToken__NotTransferable()`, async function () {
            const data = encodeFunctionData({
                ...
                args: [accountB.address, accountA.address, BigInt(licenseTokenId1)]
            });

            await expect(
                ipAccountExecute("A", licenseTokenAddress, 0, ipIdA, data, true)
            ).to.be.rejectedWith(`Failed to execute the IP Account transaction: The contract function "execute" reverted with the following signature:`, `0xd175f85c`);
        });
    ...
    describe(`Non-Commercial Social Remixing PIL: "transferable":true, "derivativesAllowed":true, "derivativesReciprocal":true`, async function () {
        ...
        step(`"transferable":true, wallet A can transfer the licenseTokenId to Wallet B`, async function () {
            const data = encodeFunctionData({
                abi: transferLicenseTokenAbi,
                functionName: "transferFrom",
                args: [accountA.address, accountB.address, BigInt(licenseTokenId1)]
            });

            const response = await ipAccountExecute("A", licenseTokenAddress, 0, ipIdA, data, true)
