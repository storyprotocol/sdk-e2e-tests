import { privateKeyA, privateKeyB, accountB, nftContractAddress } from '../../config/config'
...
describe('SDK E2E Test', function () {
    describe("Link IP to Parent IP Asset", async function () {
        ...
        describe("Link an IP asset to the parent IP asset, root IP\'s policy allows derivatives (derivativesAllowed: true)", async function () {
        ...
        describe("Link an IP asset to the parent IP asset, root IP\'s policy allows derivatives (derivativesAllowed: false)", async function () {
            step("Mint a NFT to Wallet A and get a tokenId (tokenIdA)", async function () {
                tokenIdA = await mintNFT(privateKeyA);
                expect(tokenIdA).not.empty
            });
